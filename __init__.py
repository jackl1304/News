...
def login_required(f):
    """Decorator, der sicherstellt, dass der Benutzer angemeldet ist."""
    @wraps(f)
...
def admin_required(f):
    """Decorator, der sicherstellt, dass der Benutzer ein Administrator ist."""
    @wraps(f)
    def decorated_function(*args, **kwargs):
...
def create_app(config_name=\'development\'):
    ...
    def manual_newsletter():
        try:
            ...
            logger.error(f"Fehler bei der manuellen Newsletter-Generierung: {e}")
            return jsonify({\'error\': str(e)}), 500
    ...
    def health_check():
        try:
            ...
            # email_configured = email_service.test_email_configuration() # Diese Methode existiert nicht in EmailService
            return jsonify({
                \'status\': \'healthy\',
                \'database\': \'connected\',
